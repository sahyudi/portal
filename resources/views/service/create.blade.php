    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-12">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <form method="POST" action="{{ route('save-service') }}" class="mt-6 space-y-6">
                    @csrf
                    @method('POST')
                    <div>
                    ...
                    <div>
                        ...
                        <textarea class="mt-1 block w-full" id="deskripsi" name="deskripsi" rows="10">{{ old('deskripsi') }}</textarea>

                    </div>
                    <div>
                        <x-input-label for="syarat" :value="__('Syarat')" />
                        <textarea class="mt-1 block w-full" id="syarat" name="syarat" rows="10">{{ old('syarat') }}</textarea>
